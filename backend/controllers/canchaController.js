...
class CanchaController {
  ...
  async getDisponibilidad(req, res) {
    ...
    try {
      ...
      const [reservas] = await db.query('SELECT hora_inicio FROM reservas WHERE cancha_id = ? AND fecha = ? AND estado != "cancelada"', [id, fecha]);
      ...
      for (let h = apertura; h < cierre; h++) {
        ...
        slots.push({ hora, disponible: !horasOcupadas.includes(hora), precio: cancha[0].precio_hora });
      }
  ...
  async create(req, res) {
    ...
    if (!nombre || !deporte || !precio_hora) return res.status(400).json({ error: 'Nombre, deporte y precio son requeridos' });
    try {
      const [result] = await db.query('INSERT INTO canchas (nombre, deporte, descripcion, capacidad, precio_hora, hora_apertura, hora_cierre) VALUES (?, ?, ?, ?, ?, ?, ?)', [nombre, deporte, descripcion || null, capacidad || 10, precio_hora, hora_apertura || '07:00:00', hora_cierre || '23:00:00']);
  ...
  async update(req, res) {
    ...
    try {
      await db.query('UPDATE canchas SET nombre = COALESCE(?, nombre), deporte = COALESCE(?, deporte), descripcion = COALESCE(?, descripcion), capacidad = COALESCE(?, capacidad), precio_hora = COALESCE(?, precio_hora), hora_apertura = COALESCE(?, hora_apertura), hora_cierre = COALESCE(?, hora_cierre), activo = COALESCE(?, activo) WHERE id = ?', [nombre, deporte, descripcion, capacidad, precio_hora, hora_apertura, hora_cierre, activo, req.params.id]);
